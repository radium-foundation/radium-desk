...
class RequestCorrectSerialCommunicationHistoryService
{
    ...
    private function whatsappHistory(Collection $orderIds): array
    {
        $lastSent = WhatsAppTemplateDispatch::query()
            ...
            ->where('template_key', WhatsAppTemplate::RequestCorrectSerial->value)
    ...
    private function emailHistory(Collection $orderIds): array
    {
        ...
        foreach ($logs as $log) {
            if (($log->new_values['notification_type'] ?? null) !== NotificationType::RequestCorrectSerial->value) {
                continue;
            }
    ...
    private function sentState(?CarbonInterface $sentAt): array
    {
        return [
            ...
            'status_label' => 'SENT',
            'last_sent_at' => $sentAt,
            'last_sent_label' => AppDateFormatter::format($sentAt, self::LAST_SENT_DISPLAY_FORMAT),
            'failure_reason' => null,
        ];
    }
