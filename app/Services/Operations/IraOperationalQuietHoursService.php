...
class IraOperationalQuietHoursService
{
    public function isQuietHours(?Carbon $at = null): bool
    {
        if (! (bool) config('ira.communication.quiet_hours.enabled', true)) {
            return false;
        }
        ...
        $start = (string) config('ira.communication.quiet_hours.start', '21:00');
        $end = (string) config('ira.communication.quiet_hours.end', '08:00');
