...
class OperationsSystemHealthService
{
     ...
     */
    private function automationRuntime(): array
    {
        ...
        $recentSuccess = AutomationExecution::query()
    ...
    private function queueWorker(): array
    {
        ...
        if ($snapshot->failedJobs > 0) {
            return $this->component('queue_worker', 'Queue Worker', OperationsHealthStatus::Failed, "{$snapshot->failedJobs} failed job(s) in dead-letter queue.");
        }

        if ($snapshot->pendingJobs > 50) {
            return $this->component('queue_worker', 'Queue Worker', OperationsHealthStatus::Warning, "{$snapshot->pendingJobs} pending job(s) waiting.");
        }

        if ($snapshot->oldestPendingJobAt !== null && $snapshot->oldestPendingJobAt->lt(now()->subMinutes(30))) {
     ...
     * @return array<string, mixed>
     */
    private function notificationDispatcher(): array
    {
        ...
        $todayDispatches = AuditLog::query()
            ->where('event', NotificationAuditTrailService::EVENT_DISPATCHED)
            ->where('created_at', '>=', today())
            ->count();
     ...
     * @return array<string, mixed>
     */
    private function desktopNotifications(): array
    {
        return $this->channelHealth(
            key: 'desktop',
    ...
    private function channelHealth(
        ...
        $todayFailure = $this->channelFailuresToday($channel);
    ...
    private function channelFailuresToday(string $channel): int
    {
        if (! Schema::hasTable('audit_logs')) {
            return 0;
        }
        ...
        return AuditLog::query()
            ...
            ->filter(function (AuditLog $log) use ($channel): bool {
                foreach ($this->channelResults($log) as $result) {
                    if (($result['channel'] ?? null) === $channel && ($result['success'] ?? false) === false && ($result['status'] ?? '') !== 'not_yet_configured') {
                        return true;
                    }
                }
    ...
    private function dispatchHadChannelFailure(AuditLog $log): bool
    {
        foreach ($this->channelResults($log) as $result) {
