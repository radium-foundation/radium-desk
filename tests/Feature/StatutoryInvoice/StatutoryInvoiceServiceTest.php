...
class StatutoryInvoiceServiceTest extends TestCase
{
    ...
    public function test_pos_internal_receipt_cannot_become_the_statutory_number(): void
    {
        $this->enableTestSeries();
        ...
        $stock->stockInSerialized($product, $branch, ['SN-STAT-1'], $this->actor);
        $sale = app(PosSaleService::class)->completeSale(
            branch: $branch,
            customer: ['name' => 'Walk-in', 'phone' => '9000000001'],
            lines: [['product_id' => $product->id, 'qty' => 1, 'serials' => ['SN-STAT-1']]],
            paymentMethod: 'Cash',
            actor: $this->actor,
        );
        ...
        $statutory = $this->invoices->issueFromPosSale($sale, $this->actor);

        $this->assertNotSame($sale->invoice_number, $statutory->invoice_number);
        $this->assertSame('TEST-00001', $statutory->invoice_number);
        $this->assertSame($sale->id, $statutory->inventory_sale_id);
        $this->assertSame($sale->fresh()->statutory_invoice_id, $statutory->id);
        $this->assertSame('84716050', $statutory->items->first()?->hsn_sac);
        $this->assertNull($statutory->cgst);
        $this->assertNull($statutory->finance_journal_id);
        $this->assertSame(
            1,
            FinanceJournal::query()->where('source_type', 'pos_sale')->where('source_id', $sale->id)->count(),
        );
    }
