...
const reportPath = process.env.POS_SERIAL_MERGE_QA_REPORT || "/tmp/pos-serial-cart-merge-browser-qa.json";
const chromePath =
...
(async () => {
  ...
  try {
    ...
    await page.locator("#pos-serial-results button", { hasText: "MERGE-SN-001" }).click();
    ...
    await page.locator("#pos-serial-results button", { hasText: "MERGE-SN-002" }).click();
    if (await cartRowCount(page) !== 1) {
    ...
    await page.locator("#pos-serial-results button", { hasText: "MERGE-SN-003" }).click();
    if (await cartRowCount(page) !== 1) {
      issues.push("After third serial for same model, expected one cart row.");
    }
    if ((await lastCartQty(page)) !== "3") {
      issues.push(`After third serial, expected qty 3, got ${await lastCartQty(page)}.`);
    }
    ...
    await page.locator("#pos-serial-results button", { hasText: "MERGE-SN-001" }).click();
    if ((await cartRowCount(page)) !== 1 || (await lastCartQty(page)) !== "3") {
      issues.push("Duplicate serial selection should not create another row or increment quantity.");
    }
