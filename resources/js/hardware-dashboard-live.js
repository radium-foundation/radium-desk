...
const NEEDS_ACTION_FILTERS = [
    'needs_action',
    'mapping_required',
    'awaiting_serial',
    'awb_pending',
    'package_photo_pending',
];
...
const COMPLETED_FILTERS = ['completed', 'delivered'];

const currentHardwareFilter = () => hardwareWorkspace()?.dataset.hardwareFilter ?? 'needs_action';
...
const rowMatchesActiveView = (row) => {
    ...
    if (scope === 'shipped') {
        return COMPLETED_FILTERS.includes(rowFilter)
            || rowScope === 'shipped'
            || row?.queue === 'completed';
    }

    if (rowScope === 'shipped' || COMPLETED_FILTERS.includes(rowFilter) || row?.queue === 'completed') {
        return false;
    }
    ...
    if (filter === 'needs_action') {
        return NEEDS_ACTION_FILTERS.includes(rowFilter);
    }

    if (filter === 'shipping') {
        return SHIPPING_FILTERS.includes(rowFilter);
    }

    if (COMPLETED_FILTERS.includes(filter)) {
        return COMPLETED_FILTERS.includes(rowFilter) || row?.queue === 'completed';
    }
