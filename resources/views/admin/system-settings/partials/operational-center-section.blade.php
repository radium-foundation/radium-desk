@php
    $showPlatformIntegrations = $showPlatformIntegrations ?? true;
    ...
    if (! $showPlatformIntegrations) {
        $platformCategoryKeys = ['email', 'whatsapp', 'telegram', 'outbox', 'system'];
        $featureFlagSettings = $featureFlagSettings->reject(function ($setting) use ($platformCategoryKeys) {
            $key = (string) ($setting['key'] ?? '');
            foreach ($platformCategoryKeys as $prefix) {
...
<x-system-settings.section
    ...
    description="{{ $showPlatformIntegrations ? 'Integrations, channels, and feature controls in one place.' : 'Operational feature controls for Desk administration.' }}"
>
    <div class="settings-center-op-grid mb-4">
        ...
        <x-settings-center.operational-hub-card
            ...
            status-tone="success"
            description="Runtime feature toggles across operational Desk behaviour."
            primary-label="Configure"
    ...
    <div class="settings-center-op-panels">
        ...
        <x-system-settings.card id="operational-feature-flags" title="Feature Flags" description="Enable or disable operational Desk capabilities." class="mb-3">
            <div class="system-settings-rows">
