@php
    ...
    $formAction = $formAction ?? route('refunds.index');
    $clearUrl = $clearUrl ?? route('refunds.index');
    $filters = $filters ?? [];
...
@if($embedded)
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-2 mb-3">
        <div>
            <h2 class="h5 mb-0">Refunds</h2>
            <p class="text-muted small mb-0">Operations refund queue and workflow.</p>
        </div>
        @can('create', App\Models\RefundRequest::class)
            <a href="{{ route('refunds.create') }}" class="btn btn-sm btn-primary">
...
<div class="row g-3 mb-4"
     @if($embedded) data-operations-embedded-panel="refunds" @endif>
    @foreach($queues as $queueKey => $queueMeta)
        <div class="col-6 col-lg-3">
            <a href="{{ $embedded
                    ? route('dashboard.workspace', ['workspace' => 'refunds', 'queue' => $queueKey])
                    : route('refunds.index', ['queue' => $queueKey]) }}"
               class="text-decoration-none"
               @if($embedded) data-operations-embedded-nav="refunds" @endif>
                <div @class([
...
<div class="card border-0 shadow-sm mb-4">
    ...
    <div class="card-body">
        <form method="GET"
              action="{{ $formAction }}"
              class="row g-3"
              @if($embedded) data-operations-embedded-form="refunds" @endif>
            @if($embedded)
                <input type="hidden" name="workspace" value="{{ $workspace }}">
            @endif
            @if(filled($activeQueue ?? null))
            ...
            <div class="col-12 d-flex flex-wrap gap-2">
                ...
                <a href="{{ $clearUrl }}"
                   ...
                   @if($embedded) data-operations-embedded-clear="refunds" @endif>Clear</a>
            </div>
        </form>
    </div>
</div>
...
<div class="card border-0 shadow-sm">
    ...
    @if($refunds->hasPages())
        <div class="card-footer bg-white"
             @if($embedded) data-operations-embedded-pagination="refunds" @endif>
            {{ $refunds->links() }}
        </div>
    @endif
</div>
